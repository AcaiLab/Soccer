...
def sanitize_event_name(event: str) -> str:
    """Convert event/comments_type string to a safe directory/filename component."""
...
# ----------------------------
def write_clip_and_frames(cap, fps, duration_sec, w, h, center_sec, before_sec, after_sec,
                          tag, event_name, clips_dir, frames_dir):
    ...
    # ── event-named subdirectories ──────────────────────────────────────────
    event_clips_dir  = clips_dir  / event_name
    event_frames_dir = frames_dir / event_name
    event_clips_dir.mkdir(parents=True, exist_ok=True)
    event_frames_dir.mkdir(parents=True, exist_ok=True)

    # embed event name in the mp4 filename
    clip_name = event_clips_dir / f"{event_name}_clip_{tag}_{start_sec:08.3f}s_to_{end_sec:08.3f}s.mp4"
    ...
    while idx < end_f:
        ...
        if idx in save_idxs:
            t_abs = idx / fps
            m_val = int(t_abs // 60)
            s_val = t_abs - 60 * m_val
            label = f"{m_val:02d}-{s_val:06.3f}"
            # embed event name in the png filename
            out_png = event_frames_dir / f"{event_name}_frame_{tag}_t{label}.png"
    ...
    vout.release()
    print(f"  [{event_name}] {tag}: wrote {wrote} frames → {clip_name.name}")
...
def process_match(match_folder: pathlib.Path, output_base: pathlib.Path):
    ...
    # Separate timestamps + events by half
    # Each entry: (timestamp, sanitized_event_name)
    entries_half1 = []
    entries_half2 = []
    ...
    for comment in data.get("comments", []):
        ...
        event = comment.get("comments_type", "unknown")
        event_safe = sanitize_event_name(event) if event else "unknown"

        if not ts:
            continue

        key = (ts, event_safe)
        if half == 1 and key not in seen1:
            seen1.add(key)
            entries_half1.append(key)
        elif half == 2 and key not in seen2:
            seen2.add(key)
            entries_half2.append(key)

    print(f"Found {len(entries_half1)} unique (timestamp, event) pairs for first half")
    print(f"Found {len(entries_half2)} unique (timestamp, event) pairs for second half")
    ...
    for d in (half1_clips, half1_frames, half2_clips, half2_frames):
        d.mkdir(parents=True, exist_ok=True)

    # Process halves
    if entries_half1:
        print(f"\n--- Processing First Half ---")
        process_half(video_half1_path, entries_half1, half1_clips, half1_frames)

    if entries_half2:
        print(f"\n--- Processing Second Half ---")
        process_half(video_half2_path, entries_half2, half2_clips, half2_frames)
...
# ----------------------------
def process_half(video_path: pathlib.Path, entries: list,
                 clips_dir: pathlib.Path, frames_dir: pathlib.Path):
    ...
    # Sort by timestamp
    entries_sorted = sorted(entries, key=lambda e: parse_time_stamp(e[0]))

    for ts, event_safe in entries_sorted:
        try:
        ...
        write_clip_and_frames(cap, fps, duration_sec, w, h, center_sec,
                              CLIP_BEFORE_SEC, CLIP_AFTER_SEC,
                              tag, event_safe, clips_dir, frames_dir)
